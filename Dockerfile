...
RUN cd /tmp && \
    ...
    ./b2 install
#not used right now but maybe later when i understand what to do with it
RUN cd /tmp && \
...
COPY ./geoflow_resources/resources/ /dim_pipeline/resources
COPY ./geoflow_resources/code/run.py /dim_pipeline/python
